...
int main(int argc, const char** argv)
{
	...
	for (;;)
	{
		...
		if (!paused)
		{
			cvtColor(image, hsv, COLOR_BGR2HSV); //把图像从RGB表色系转为HSV表色系

			if (trackObject)  //track_object非零,表示有需要跟踪的物体
			{
				...
				if (trackObject < 0)//鼠标选择区域松开后，该函数内部又将其赋值-1
				{
					...
					for (int i = 0; i < hsize; i++)
					{
						...
						rectangle(histimg, Point(i*binW, histimg.rows),						//在一幅输入图像上画一个简单抽的矩形
							Point((i + 1)*binW, histimg.rows - val),						//指定左上角和右下角，并定义颜色，大小，线型等
				...
				RotatedRect trackBox = CamShift(backproj, trackWindow,				//trackWindow为鼠标选择的区域，TermCriteria为确定迭代终止的准则
					TermCriteria(TermCriteria::EPS | TermCriteria::COUNT, 10, 1));	//CV_TERMCRIT_EPS是通过forest_accuracy,CV_TERMCRIT_ITER
